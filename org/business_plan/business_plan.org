...
#+BEGIN_abstract
This document covers the high-level overview for what we have an intend to accomplish as an organization.
...
For details about the rationale behind the overarching message of Universal Basic Philanthropy,
please consult our whitepaper
[[cite:dinh2020universal]] or explore supplementary media at https://tokenibis.org.
#+END_abstract
...
We believe that are three major shortcomings of the current institution:

- *Power:* Social issues and services affect everyone, but decision-making power depends on private wealth.
- *Autonomy:* Individuals seeking services need help, but unilaterally help undermine the self-determinism of already marginalized populations.
- *Information:* In the current setup, it is difficult for existing institutions to quantity which organizations an causes require the most attention.

We believe that the silver bullet to cut through these shortcomings is to explicitly place the decision-making into the hands of everyday citizen.
...
The mission of Token Ibis is to explore this radical model at a feasible score.
...
3. *Engage*: By giving them a financial voice, we encourage users to earn more money, deposit their own, or engage with nonprofits in even more ways.
...
/We do *not* necessarily claim to have official partnerships with the organization itself./
...
\avatar{figures/avatars/cabq}{City of Albuquerque}{As an Albuquerque-based organization, we have an interested in working with the city government to promote Token Ibis as a platform for community engagement.
...
\avatar{figures/avatars/storehouse}{Storehouse}{Operates the state's largest food pantry, which offering free food in downtown Albuquerque.}
...
\avatar{figures/avatars/treenm}{Tree New Mexico}{Operations community-oriented programs to maintain and grow the city's urban forestry.}
...
As of the writing of this document, Token Ibis is midway through Phase I and preparing to scope out phase II.
...
The Token Ibis team, support base, and digital platform have all remained active and continuous through both pivots.
We feel that the current iteration of UBP and the surrounding business model are beginning to solidify.

** Phase II: Scale
Having committed to a watertight message and business model, the next challenge to scale up the ecosystem.
This process entails growing three key resources:
...
UBP can be deployed by any organization that has funding, nonprofit connections, and population of interested users---features that describe most foundations and United Ways.
On our end, Token Ibis can supply free software, marketing materials, and lessons learned.
In return, we will encourage organizations to allow us to aggregate public donor data so that we can pursue even more meaningful analysis at scale.
...
The overview is simple: Token Ibis has next to overhead, so we are free to re-purpose all incoming donor money into philanthropy dollars for our users.
...
But every now and then, there are radical, actionable ideas where even failure is impossible.

As long as we are right about the value of UBP, any work that we do will only serve as a building block for others to build on.
...
Thanks to our heavy reliance on automation, flexible economic model, and streamline organizational structure, we don't have any hard capital needs.
Our operational costs are negligible, and our board has committed to enough bare minimum funding that Token Ibis will move meaningful philanthropy money (at least $2,000 per month) indefinitely.

Instead, you can think of Token Ibis as a hyper-efficient engine for converting dollars into impact.
